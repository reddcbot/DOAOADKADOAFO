...
module.exports.run = async (client, message, args) => {
  ...
  if(!message.member.roles.has('606148649717596170')) return message.reply('Kayıt için yetkiniz yok!')
    ...
    let role = msg.guild.roles.find(r => r.name === "「♀」Ladies");
    if (!role) {
        try {
    ...
    await kayıt.removeRole('606137353546760193').catch()
    msg.channel.send({
        embed: {
            color: Math.floor(Math.random() * (0xFFFFFF + 1)),
