...
module.exports.run = async (client, message, args) => {
  ...
  if(!message.member.roles.has('584093503957499945')) return message.reply('Rol vermek için yetkiniz yok!')
    ...
    let role = msg.guild.roles.find(r => r.name === "⭐️ | HAFTANIN ÜYESİ");
    if (!role) {
        try {
    ...
    if (kayıt.roles.has(role.id)) return msg.channel.send({
        embed: {
            color: Math.floor(Math.random() * (0xFFFFFF + 1)),
            description: (" 💡 Zaten önceden hatanın üyesi seçilmişsiniz. ")
        }
    });
    ...
    msg.channel.send({
        embed: {
            color: Math.floor(Math.random() * (0xFFFFFF + 1)),
            description: (":unlock: Haftanın Üyesi rolü başarıyla eklendi.")
        }
    })
}
exports.conf = {
...
exports.help = {
    name: 'haftanınüyesiekle',
    description: 'Sunucuya kayıt olursunuz!',
    usage: 'haftanınüyesiekle'
};